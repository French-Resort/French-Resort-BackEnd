...
from services import GuestService, BookingService, RoomService, UserService
from api import api_bp, LoginResource, SignUpResource, BookingResource, BookingsResource
...
def index():
    ...
    if form.validate_on_submit():
        user = UserService.authenticate_user_admin(form.user_email.data, form.user_password.data)

        if not user:
            return render_template('/pages/login.html', form=form, error="Invalid email or password, or user is not admin")
